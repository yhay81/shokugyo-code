...
if ($KeyResponse.StatusCode -ne 200 -or $KeyResponse.Content.Trim() -ne $Key) { throw "Published IndexNow key unavailable" }
$Map = Invoke-WebRequest -Uri "$NormalizedBaseUrl/sitemap.xml" -SkipHttpErrorCheck -TimeoutSec 30
$Urls = @([regex]::Matches($Map.Content, "<loc>([^<]+)</loc>") | ForEach-Object { $_.Groups[1].Value })
...
[ordered]@{ submitted_at=(Get-Date).ToUniversalTime().ToString("o"); service=([uri]$NormalizedBaseUrl).Host; status=[int]$Response.StatusCode; url_count=$Urls.Count } | ConvertTo-Json
